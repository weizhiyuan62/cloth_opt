...
The diagonal-fold policy position-controls only the single vertex selected by
`controlled_corner`. The default 10 x 10 grid has a real 10-vertex diagonal
crease, and only that crease is pinned. The other 44 vertices in the moving
triangle remain physical and follow through gravity, springs, and constraints.
The default rule pulls the top-right corner around the main diagonal (top left
to bottom right):
...
At release, the corner controller is cleared without pinning the moving
